...
class formLoader:
    ...
    def open_form(self, fields):
        html = '''<form action="{0}" method="post" accept-charset="utf-8" role="form" novalidate="novalidate" >'''.format(
            self.action)
    ...
    def element_checkboxes(self, field):
        ...
        for i in range(len(field['choices'])):
            ...
            html += '''<input type="checkbox" name="{0}_{1}" id="{0}-{1}" value="{2}" {3}>{4}'''.format(id, i, field[
                'choices'][i]['value'], checked, field['choices'][i]['title'])
            html += '''</label></div>'''
    ...
    def element_multiple_choice(self, field):
        ...
        for i in range(len(field['choices'])):
            ...
            html += '''<input type="radio" name="{0}" id="{0}_{1}" value="{2}" {3}>{4}'''.format(id, i,
                                                                                                 field['choices'][i][
                                                                                                     'value'], checked,
                                                                                                 field['choices'][i][
                                                                                                     'title'])
